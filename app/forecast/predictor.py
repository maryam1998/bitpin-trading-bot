...
class Predictor:
    ...
    def generate_prediction_report(self, symbol: str, historical: List[Dict], steps: int = 30,
                                    is_synthetic: bool = False) -> Dict[str, Any]:
        """
        تولید گزارش کامل پیش‌بینی

        is_synthetic: وقتی True باشد یعنی داده‌ی تاریخی ورودی واقعی نیست
        (مثلاً برای طلا/دلار که تاریخچه‌ی واقعی در دسترس نبوده و به‌صورت
        نوسان تصادفی حول قیمت فعلی ساخته شده است). در این حالت سطح
        اطمینان همیشه «پایین» گزارش می‌شود تا این پیش‌بینی هرگز به عنوان
        یک سیگنال معاملاتیِ مطمئن به کاربر نشان داده نشود.
        """
        if not historical:
        ...
        if is_synthetic:
            confidence = "پایین"
            recommendation = "HOLD"
            direction += " (⚠️ بر پایه‌ی داده‌ی تخمینی، نه تاریخچه‌ی واقعی)"
        else:
